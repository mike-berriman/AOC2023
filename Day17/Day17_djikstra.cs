    ...
    internal class DjikstraClass
    {
        ...
        public long Calculate1()
        {
            ...
            for (int i = 0; i < Weights.GridHeight; i++)
            {
                ...
                {
                    long val = VisitedCache.Keys.Where(x => (x.Coord.X == j) && (x.Coord.Y == i)).OrderBy(x => x.Distance).First().Distance;
            ...
            total = VisitedCache.Keys.Where(x => (x.Coord.X == (Weights.GridWidth - 1)) && (x.Coord.Y == (Weights.GridHeight - 1))).OrderBy(x => x.Distance).First().Distance;
